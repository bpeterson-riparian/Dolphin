...
def process_document(document_path, config_path, save_dir, max_batch_size):
...
def main():
    ...
    run_model(args.config, args.input_path, args.save_dir, args.max_batch_size)


def run_model(config_path, document_path, save_dir, max_batch_size):
    if not os.path.exists(document_path):
        raise FileNotFoundError(f"Input path {document_path} does not exist")

    document_files = get_document_files(document_path)

    save_dir = save_dir or (
        document_path
        if os.path.isdir(document_path)
        else os.path.dirname(document_path)
    )
    ...
    with Pool(processes=4) as pool:
        pool.starmap(
            try_to_process_document,
            [
                (file_path, config_path, save_dir, max_batch_size)
...
def try_to_process_document(file_path, config_path, save_dir, max_batch_size):
    try:
        return process_document(file_path, config_path, save_dir, max_batch_size)
    except Exception as e:
        print(f"Error processing {file_path}: {str(e)}")
        return None


def get_document_files(input_path):
